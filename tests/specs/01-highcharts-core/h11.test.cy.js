describe("template spec", () => {
  beforeEach(() => {
    cy.visit(
      "../../../exercises/01-highcharts-core/11-histogram-point-select/index.html"
    );
  });
        ...
        for (const axis of chart.axes) {
          if (axis.opposite) {
            expect(
              ...
              `The opposite ${axis.coll} should have the title 'Histogram'`
            ).to.equal("Histogram");
          } else {
            expect(
              axis.options.title.text,
              `The normal ${axis.coll} should have the title 'Values'`
            ).to.equal("Values");
          }
        }
      });
  ...
  it("should check if there is a scatter series joined to the histogram series", () => {
    cy.window()
      ...
      .then((Highcharts) => {
        const chart = Highcharts.charts[0];
        const histogramSeries = chart.series.filter(
        ...
        expect(
          histogramSeries,
          "There should be one histogram series"
        ).to.have.length(1);
        expect(
          scatterSeries,
        ...
        expect(
          histogramSeries[0].baseSeries,
          "The histogram series should be joined to the scatter series"
        ).to.equal(scatterSeries[0]);
      });
  });
  ...
  it("should check if the histogram series is selectable", () => {
    cy.window()
      ...
      .then((Highcharts) => {
        const chart = Highcharts.charts[0];
        const histogramSeries = chart.series.find(
          (series) => series.type === "histogram"
        );

        expect(
          histogramSeries.options.allowPointSelect,
  ...
  it("should check if the inactive state is turned off for all the series", () => {
    cy.window()
      ...
      .then((Highcharts) => {
        ...
        expect(
          allInactiveStatesDisabled,
          "All the series should have the inactive state turned off"
        ).to.be.true;
      });
  });
  ...
  it("should check if clicking on the histogram column selects the proper points", () => {
    ...
    cy.get(".highcharts-histogram-series .highcharts-point-select").should(
      "have.length",
      1
    );
  });
