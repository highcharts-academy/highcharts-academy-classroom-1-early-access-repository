describe("06-plotLine-plotBand", () => {
  beforeEach("passes", () => {
    cy.visit(
      "../../../exercises/01-highcharts-core/06-plotLine-plotBand/index.html"
    );
  });
  ...
  it("should check if plotLine is added correctly with proper max value", () => {
    cy.window()
      ...
      .then((Highcharts) => {
        ...
        expect(
          plotLine.value,
          "PlotLine value should be 1.5x the max value"
        ).to.be.closeTo(chart.yAxis[0].dataMax * 1.5, 0.1);

        // Ensure the plotLine is rendered in the DOM
        cy.get("#container svg path.highcharts-plot-line")
          .should("exist")
  ...
  it("should add plotBand with correct range", () => {
    cy.window()
      ...
      .then((Highcharts) => {
        ...
        expect(
          plotBand.from,
          "PlotBand 'from' value should be 0.5x the max value"
        ).to.be.closeTo(chart.yAxis[0].dataMax * 0.5, 0.1);
        expect(
          plotBand.to,
          "PlotBand 'to' value should be 1.2x the max value"
