...
describe("08-accessible-chart-tests", () => {
  beforeEach("passes", () => {
    cy.visit(
      "../../exercises/05-highcharts-accessibility/02-accessible-colors/index.html"
    );
  });
  ...
  it("Columns should have enough contrast to background color of the chart.", () => {
    cy.window()
      ...
      .then((Highcharts) => {
        ...
        const contrastBlueSeries = calculateContrastRatio(
          blueSeriesColor,
          backgroundColor
        );
        const contrastPurpleSeries = calculateContrastRatio(
          purpleSeriesColor,
          backgroundColor
        );
