describe('05-finding-max', () => {
  beforeEach('passes', () => {
    cy.visit('../../../exercises/highcharts-core/05-finding-max/index.html');
  });
  ...
  it('should check if there are 3 series and each series has 3 random data points', () => {
    cy.window().its('Highcharts').then(Highcharts => {
      ...
      for (const series of chart.series) {
        ...
        expect(series.points, `Series '${series.name}' should have 3 points`).to.have.length(3);

        // Check all points have values between 0 and 9
        for (const point of series.points) {
          expect(point.y, `Point value in series '${series.name}' should be between 0 and 9`).to.be.gte(0).and.to.be.lte(9);
        }
      }
    });
  });
