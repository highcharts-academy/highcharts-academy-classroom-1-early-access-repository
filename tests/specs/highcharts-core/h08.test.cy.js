    ...
    cy.window()
      ...
      .then((Highcharts) => {
        ...
        const largeSeries = columnSeries.find(
          (series) => series.xData.length >= 10000,
        );
        expect(
          largeSeries,
          "One scatter series should have at least 10,000 points",
        ).to.exist;
        expect(largeSeries.boosted, "This series should be in boost mode").to.be
          .true;
      });
  });
});
